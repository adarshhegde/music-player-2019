...
class meta {
    ...
    buildMeta(force_purge = false) {
        ...
        return Promise.all(promises).then(resolvedPromises => {
            ...
            resolvedPromises.map(x => {
                ...
                if (!this.data[info.artist].albums[info.album].tracks.hasOwnProperty(hashed)) {
                    let NewTrack = new Track(info, this.data[info.artist].albums[info.album]);
                    this.data[info.artist].albums[info.album].tracks[hashed] = NewTrack;
                    this.data[info.artist].albums[info.album].tracks[hashed].path = temppath;
                    this.data[info.artist].albums[info.album].tracks[hashed].uid = hashed;
                    this.data[info.artist].albums[info.album].tracks[hashed].track_index = i++;
                    this.data[info.artist].albums[info.album].track_count++;
                    this.data[info.artist].albums[info.album].tracks[hashed].track_pos = Object.keys(this.data[info.artist].albums[info.album].tracks).length;

                    if (info.picture && info.picture.length > 0) {
                        this.handleThumbnails(this.data[info.artist].albums[info.album].tracks[hashed], info.picture[0].data, this.data[info.artist].albums[info.album], force_purge);
                    } else {
                        this.log.flip4 += 1;
                    }
                }
    ...
    getTracks() {
        ...
        let EH = Object.keys(out);
        EH.sort(function(a,b) {
            if(out[a].track_name > out[b].track_name) return -1;
            else if(out[b].track_name > out[a].track_name) return 1;
            return 0;
        });
